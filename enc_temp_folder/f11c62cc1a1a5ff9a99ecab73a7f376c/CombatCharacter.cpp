...
void ACombatCharacter::BeginPlay()
{
	...
	ComboMax = AttackComboMontages.Num();
}
...
{
	if (ComboMax == 0) return;

	if (ComboIndex > 0 && ComboIndex < ComboMax)
	{
		AnimInstance->Montage_Stop(0.3f, AttackComboMontages[ComboIndex - 1]);
		AnimInstance->Montage_Play(AttackComboMontages[ComboIndex]);
	}
	else
	{
		ComboIndex = 0;
		AnimInstance->Montage_Play(AttackComboMontages[0]);
	}
